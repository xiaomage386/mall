...
let LOCAL_SERVER_URL = HTTP_CLIENT.LOCAL_SERVER ? HTTP_CLIENT.LOCAL_SERVER : (HTTP_CLIENT.LOCAL_SERVER + HTTP_CLIENT.SERVER_RUNTIME || '');
if (Utils.size(url) > 0) {
...
let localActionMethods = (function() {
	if (HTTP_CLIENT.URL_MODEL == 1) {
		return function (action) {
            action = action.split('.');
			let urlPath = '/' + action.join('/');
			return LOCAL_SERVER_URL + urlPath + URL_EXT;
		}
	}
	return function (action) {
		action = action.split('.');
		return LOCAL_SERVER_URL + '?' + HTTP_CLIENT.VAR_MODULE +
			'=' + action[0] + '&' + HTTP_CLIENT.VAR_CONTROLLER +
			'=' + action[1] + '&' + HTTP_CLIENT.VAR_ACTION +
			'=' + (action[2] ? action[2] : '') + URL_EXT;
	}
...
let ajaxMethods = function (method, action, extraParams) {
	...
	defer.promise.execute = function() {
		...
		let actionURL = extraParams.unURL ? action : actionMethods(action)
		if (extraParams.localLogin == 2) { // 本地服务器访问
			actionURL = extraParams.unURL ? action : localActionMethods(action)
		}
		// 合并参数
		...
		// 查询字符串
		extraParams.data = qs.stringify(extraParams.data, {arrayFormat: 'brackets'});
		...
		httpPromise.then(function(response) {
			...
			if (data[AUTH_TIMEOUT_FIELD] == AUTH_TIMEOUT_CODE) {
				...
				console.log('yes')
				// Promise.all(executeInterceptor(httpClient.interceptor.responseIntercepts, response)).then(function(){
...
// 配置全局请求参数
httpClient.defaults(HTTP_CLIENT['axios'] || '');
...
/* function getRefreshToken() {
	...
	return axios.post('auth2/accessToken', qs.stringify(data))
}
